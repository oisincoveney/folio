...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 folio/folio.py composes all four components and registers the index page with on_load=AppState.load_models
- [ ] #2 app.py, templates/, and static/ are deleted
- [ ] #3 pyproject.toml contains no reference to flask
- [ ] #4 `reflex run` is the only command needed to start the app
- [ ] #5 Drag-and-drop upload triggers parse and streams results to the UI (FOLIO-9 AC #2)
- [ ] #6 Save flow writes the PDF and appends to payments.csv (FOLIO-9 AC #3)
- [ ] #7 Retry flow re-parses a failed file from the UI (FOLIO-9 AC #4)
- [ ] #8 parse.py, storage.py, normalization.py, config.py are unmodified (FOLIO-9 AC #6)
<!-- AC:END -->
